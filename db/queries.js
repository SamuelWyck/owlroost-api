...
async function getUserProfile(userId, requestingUser) {
    ...
    if (requestingUser) {
        [user, follow_requests] = await Promise.all([
            ...
            pool.query("SELECT fr.id AS request_id, su.id AS sending_user_id, su.username AS sending_user_username, su.profile_img_url AS sending_user_img FROM follow_requests AS fr JOIN users AS su ON su.id = fr.requesting_user_id WHERE fr.receiving_user_id = $1 OR fr.requesting_user_id = $1", [userId])
        ]);
